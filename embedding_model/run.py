...
model_name = 'relu_vpnn_lr%f_masking_layer_layer%d' % (Config.lr, Config.num_layers)
...
def stack_spectrograms(spec):
    ...
    # padding_removed = tf.slice(stacked, [1, 0, 0], [Config.num_time_frames, -1, -1]) # get rid of padding
    # padding_removed = tf.Print(padding_removed, [padding_removed.get_shape()])
    # print 'padding_removed shape: %s' % (padding_removed.get_shape())
...
def prepare_data(train):
    ...
    # normalize data
    stats = None #np.load(os.path.join(PREPROCESSING_STAT_DIR, 'stats.npy'))
